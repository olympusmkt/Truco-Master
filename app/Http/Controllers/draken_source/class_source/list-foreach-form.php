...
foreach ($json_inputs['inputs'] as $key => $value) {
    ...
    if ($value['type'] == "textarea") {
        ...
        $form_html .= '<div class="uk-width-1-1@s">
                            ...
                            <textarea class="uk-textarea" rows="5" type="'.$value['type'].'" id="'.$value['id'].'" placeholder="'.$value['placeholder'].'">{{ $details[0]->'.$value['id'].' }}</textarea>
                        </div>
        ';
    } else {
        ...
        $form_html .= '<div class="uk-width-1-1@s">
                            <label>'.$value['placeholder'].'</label>
                            <input class="uk-input" type="'.$value['type'].'" id="'.$value['id'].'" placeholder="'.$value['placeholder'].'" value="{{ $details[0]->'.$value['id'].' }}">
                        </div>
                        ';
    }
